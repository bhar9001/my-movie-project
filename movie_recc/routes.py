from flask import Blueprint, current_app, redirect, session, render_template, request, url_for, abort
import requests
import pandas as pd
...
corrMatrix = pd.read_pickle('movie_recc/static/corrMatrix.pkl')
pivot_table = pd.read_pickle('movie_recc/static/pivotKNN.pkl')
movies = pd.read_pickle('movie_recc/static/movies.pkl')

#LOAD DATASETS - CSV
links = pd.read_csv('movie_recc/static/dataset/links.csv')
movieLens = pd.read_csv('movie_recc/static/dataset/movies.csv')
tmdb_movies = pd.read_csv('movie_recc/static/tmdb_5000_credits.csv')
clean_coll = pd.read_csv('movie_recc/static/dataset/coll_clean_data.csv')
...
pages = Blueprint("movies", __name__, template_folder="templates", static_folder="static")
...
@pages.route("/recommend", methods=['GET', 'POST'])
def main():
    if not session.get("email"):
        return redirect(url_for("movies.login"))
...
@pages.route("/home/<val>",methods=['GET', 'POST'])
def got_rate(val):
    if not session.get("email"):
        return redirect(url_for("movies.login"))
    if request.method == 'POST':
